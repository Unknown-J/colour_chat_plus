...
core.register_chatcommand("4c", {
	...
	func = function(param)
		...
		local step = 125 / param:len()
 		local hue = 0
     		 -- iterate the whole 125 degrees
		local output = ""
      		for i = 1, param:len() do
			local char = param:sub(i,i)
			if char:match("%s") then
...
core.register_chatcommand(".2c", {
	...
	func = function(param)
		...
		local step = 45 / param:len()
 		local hue = 0
     		 -- iterate the whole 45 degrees
		local output = ""
      		for i = 1, param:len() do
			local char = param:sub(i,i)
			if char:match("%s") then
...
core.register_chatcommand("lc", {
	...
	func = function(param)
		...
		local step = 75000 / param:len()
 		local hue = 0
     		 -- iterate the whole 7500 degrees
		local output = ""
      		for i = 1, param:len() do
			local char = param:sub(i,i)
			if char:match("%s") then
...
core.register_chatcommand("sc", {
	...
	func = function(param)
		...
		local step = 65000 / param:len()
 		local hue = 0
     		 -- iterate the whole 650 degrees
		local output = ""
      		for i = 1, param:len() do
			local char = param:sub(i,i)
			if char:match("%s") then
